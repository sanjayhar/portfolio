...
export function Services() {
  ...
  return (
    <section ref={sectionRef} id="services" className="py-24 bg-transparent relative overflow-hidden">
        ...
        <div ref={gridRef} className="grid md:grid-cols-3 gap-6">
          {services.map((service) => {
            return (
              <article
                key={service.id}
                className="card-h group rounded-2xl p-8 border border-zinc-200/50 dark:border-zinc-800/40 bg-white/40 dark:bg-zinc-900/30 backdrop-blur-md hover:border-accent/60 hover:shadow-[0_0_45px_rgba(255,107,43,0.14)] transition-all duration-300"
              >
                <div className="icon-pop w-12 h-12 flex items-center justify-center rounded-xl mb-6 bg-orange-50 dark:bg-zinc-800/60 group-hover:bg-accent/10 transition-colors">
                  <ServiceIcon icon={service.icon} />
                </div>
                <h3 className="font-display font-bold text-xl mb-3 text-zinc-900 dark:text-white">
                  {service.title}
                </h3>
