        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div ref={imageRef} className="order-2 md:order-1">
            <div className="relative">
              <div className="p-1 rounded-3xl bg-gradient-to-tr from-accent/20 via-zinc-200/10 to-accent/20 dark:from-accent/30 dark:via-zinc-800/10 dark:to-accent/30 backdrop-blur-sm max-w-sm mx-auto">
                <PhotoFrame
                  src={portfolio.aboutImage}
                  alt={portfolio.name}
                  width={700}
                  height={700}
                  frameClassName="w-full aspect-square rounded-[20px]"
                />
              </div>
              {/* floating glow ring */}
              ...
              <div className="flex flex-wrap gap-2" role="list" aria-label="Skills">
                {skills.map((skill) => (
                  <span
                    key={skill}
                    role="listitem"
                    className="text-sm bg-white/40 dark:bg-zinc-900/30 backdrop-blur-md border border-zinc-200/50 dark:border-zinc-800/40 text-zinc-700 dark:text-zinc-300 px-3.5 py-1.5 rounded-full hover:border-accent hover:text-accent hover:shadow-[0_0_15px_rgba(255,107,43,0.2)] hover:scale-[1.03] transition-all duration-200 cursor-default"
                  >
                    {skill}
                  </span>
                ))}
