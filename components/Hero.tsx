...
export function Hero() {
  return (
    <section id="hero" className="relative min-h-screen flex items-center pt-16 overflow-hidden bg-transparent">
      {/* Subtle overlay gradients for depth, blending into the 3D scene */}
      <div
        className="absolute top-1/4 right-0 w-96 h-96 bg-accent/5 rounded-full blur-3xl pointer-events-none blob-drift"
        aria-hidden="true"
      />
      <div
        className="absolute bottom-1/4 left-0 w-64 h-64 bg-zinc-200/20 dark:bg-zinc-800/10 rounded-full blur-3xl pointer-events-none blob-drift-slow"
        aria-hidden="true"
      />
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="reveal d1 font-display font-bold text-5xl md:text-6xl lg:text-7xl leading-[1.05] tracking-tight text-zinc-900 dark:text-white mb-6">
              Hi, I&apos;m <span className="bg-gradient-to-r from-accent via-[#ff8f5c] to-accent bg-clip-text text-transparent">{portfolio.name}</span>
            </h1>
            ...
            <div className="reveal d3 flex flex-wrap gap-4">
              ...
              <Magnetic>
                <a
                  href="#contact"
                  className="inline-flex items-center gap-2 border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 font-medium px-7 py-3.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-900/50 transition-colors text-sm"
                >
                  Get in touch
                </a>
              </Magnetic>
            </div>
            <div className="reveal d4 flex gap-8 mt-14 pt-8 border-t border-zinc-150 dark:border-zinc-900/60">
          ...
          <div className="reveal d2 flex justify-center md:justify-end">
            <div className="relative w-72 h-72 md:w-80 md:h-80 lg:w-96 lg:h-96 float-y">
              <div className="w-full h-full rounded-3xl p-1 bg-gradient-to-tr from-accent/20 via-zinc-200/10 to-accent/20 dark:from-accent/40 dark:via-zinc-800/20 dark:to-accent/40 backdrop-blur-sm">
                <PhotoFrame
                  ...
                  height={800}
                  priority
                  frameClassName="w-full h-full rounded-[20px]"
                />
              </div>
